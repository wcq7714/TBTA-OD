...
def blend_image(original_image, generated_images, eta):
    ...
    for j, img in enumerate(generated_images):
        ...
        # blended_array = blended_image.squeeze(0).permute(1, 2, 0).cpu().numpy()
        # blended_array = np.clip(blended_array, 0, 255).astype(np.uint8)
        
        # blended_img = Image.fromarray(blended_array)
        # blend_img_path = os.path.join("/home/wcq/EBAD-main/imageshow/", 
        #                             f"blend_{current_time}_{j}.jpg")
        # blended_img.save(blend_img_path)
        # print(f"保存混合图像: {blend_img_path}")
...
def PM_tensor_weight_balancing(im, adv, target, w, ensemble, eps, n_iters, alpha, dataset='voc', weight_balancing=False):
    ...
    for i in range(n_iters):
        ...
        blended_images=blend_image(adv,generated_images,eta)
        loss_list = []
        loss_list_np = []
        ...
        with torch.no_grad():
            ...
            pert = pert - alpha*torch.sign(final_grad)
